...
//			    $service = strtolower($value['name']);
        ...
        switch($request_data['payment_mode']){
            ...
            case 'paymentPaypal':
                ...
                if(empty($paypal_response['status'])) {
                    (new KCAppointment())->delete(['id' => $appointment_id]);
                    (new KCAppointmentServiceMapping())->delete(['appointment_id' => $appointment_id]);
                }
	...
	public function delete() {
		...
		try {
            ...
            //appointment cancel mail to patient
            ...
            //googlemeet telemed entry delete
            if(isKiviCareGoogleMeetActive()){
                apply_filters('kcgm_remove_appointment_event',['appoinment_id' => $id]);
            }

            //appointment encounter delete
            $encounter_id = (new KCPatientEncounter())->get_var(['appointment_id' => $id],'id');
            $this->appointmentEncounterDelete($id);
            // hook for appointment before cancelled
            do_action( 'kc_appointment_delete', $id, (int)$encounter_id);
...
//            $telemedPluginActive = isKiviCareTelemedActive();
			...
			if(!empty($request_data)){
				...
				foreach($request_data['id'] as $id){
					if(!empty($id)){
                        ...
                        if (isKiviCareTelemedActive()) {
                            //delete zoom meeting
                            apply_filters('kct_delete_appointment_meeting', $id);
                        }
    ...
    public function appointmentEncounterDelete( $id){
        $encounter_id = (new KCPatientEncounter())->get_var(['appointment_id' => $id],'id');
        if(!empty($encounter_id)){
            do_action('kivicare_encounter_delete',$encounter_id);
            $encounterBill = (new KCBill())->get_var(['encounter_id' => $encounter_id],"id");
            if(!empty($encounterBill)){
                $this->db->query("DELETE FROM {$this->db->prefix}kc_bill_items WHERE bill_id IN ({$encounterBill})");
                (new KCBill())->delete(['id' => $encounterBill]);
            }
            (new KCPatientEncounter())->delete(['id' => $encounter_id]);
        }
    }
