        ...
        switch ($request_data['module']) {
            ...
            case 'receptionists':
                ...
                switch ($request_data['action_perform']) {
                    ...
                    case 'active':
                        $ids = implode(",", $ids);
                        wp_query_builder()->from('users')->set(['user_status' => 0])->where([
                            'raw' => "ID IN ({$ids})",
                        ])->update();
                        break;
                    case 'inactive':
                        $ids = implode(",", $ids);
                        wp_query_builder()->from('users')->set(['user_status' => 1])->where([
                            'raw' => "ID IN ({$ids})",
                        ])->update();
                        break;
                    case 'resend_credential':
            ...
            case 'patient_encounter_list':
                ...
                foreach ($ids as $id) {
                    switch ($request_data['action_perform']) {
                        case 'delete':
                            $encounterBill = (new KCBill())->get_var(['encounter_id' => $id],"id");
                            ...
                            do_action('kivicare_encounter_delete',$id);
                            $patient_encounter->delete( [ 'id' => $id ] );
                            $response = [
                                'status' => true,
