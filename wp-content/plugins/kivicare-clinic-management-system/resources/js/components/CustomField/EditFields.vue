<template>
  <div class="row kc-custom-field" v-if="customFields.length > 0">
    <div v-for="(field, index) in customFields" class="col-md-5 mb-3" :key="index" v-if="field.status !== undefined && field.status == '1'">
      ...
      <div v-if="field.type === 'calendar'">
        <input type="date" v-model="fieldsObj['custom_field_'+field.id]" id="doc_birthdate" @input="handleValueChange" :disabled="disabledForPatient">
      </div>
      <textarea
      ...
      ></multi-select>
      <div v-if="parseInt(field.isRequired) === 1" class="custom_field-required d-none"  :id="'custom_field_'+field.id">
        <div  class="invalid-feedback" > {{ field.label + ' ' + formTranslation.common.required}}
        </div>
      </div>
    </div>
  </div>
...
export default {
    ...
    methods: {
        init: function () {       
            if (this.module_type !== "") {
                if(this.module_type == 'appointment_module'){
        ...
        if (this.customFields.length > 0) {
            this.customFields.map((field) => {
                if (field.type === "checkbox") {
                    this.fieldsObj['custom_field_'+field.id] = field.field_data ? field.field_data :[];
    ...
    watch: {
        fieldsValue: function (newObj) {
            if(this.module_type !== 'patient_encounter_module'){
...
.kc-custom-field .custom-radio .custom-control-input ~ .custom-control-label {
    height: auto;
}
.kc-custom-field .custom-checkbox .custom-control-input~.custom-control-label {
    height: auto;
}
</style>