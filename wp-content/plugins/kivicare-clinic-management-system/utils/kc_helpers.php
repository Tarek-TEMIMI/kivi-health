...
function kcGetCustomFields($module_type, $module_id, $data_module_id = 0, $exclude_null_field_data = false)
{
    ...
    switch ($module_type) {
        case 'doctor_module':
            $query = "SELECT p.*, u.fields_data " .
                ...
                "LEFT JOIN (SELECT * FROM {$custom_field_data_table} WHERE module_id=" . $module_id . " ) AS u ON p.id = u.field_id WHERE p.status !=0 AND p.module_type =" . $type;
            break;
        case 'patient_module':
            ...
            $query = "SELECT p.*, u.fields_data " .
                "FROM {$custom_field_table} AS p " .
                "LEFT JOIN (SELECT * FROM {$custom_field_data_table} WHERE module_id=" . $module_id . " ) AS u ON p.id = u.field_id WHERE p.status !=0 AND p.module_type =" . $type . $id;
            break;
    ...
    if (is_array($data) && count($data) > 0) {
        $data = array_map(function ($v) {
            if (!empty($v['type']) && in_array($v['type'],[ 'checkbox','file_upload','multiselect'])) {
...
?>
            ...
            ?>
                ...
                <div class="form-group kivi-col-6 mt-2 <?php echo esc_html($customField['type']); ?>">
                    <label class="form-label" for="<?php echo esc_html($customField['name'] . '_' . $customField['id']); ?>" class="form-control-label mb-2">
                    ...
                    ?>
                        <input id="<?php echo esc_html($customField['name'] . '_' . $customField['id']); ?>" placeholder="<?php echo esc_html($customField['placeholder']); ?>" name="custom_field_<?php echo esc_html($customField['id']); ?>" type="<?php echo esc_html($customField['type'] === 'calendar' ? 'date' : $customField['type']); ?>" class="iq-kivicare-form-control <?php echo esc_html(in_array($customField['isRequired'], [1, '1']) ? 'kivicare-required' : ''); ?>" />
                    <?php
                    }
                    if ($customField['type'] === 'file_upload') {
                        ...
                        ?>
                            <input
                             id="<?php echo esc_html($customField['name'] . '_' . $customField['id']); ?>"
                             ...
                             accept="<?php echo esc_html($supported_file_type); ?>"
                             onchange="kivicareFileUploadSizeCheck(event,this)"
                             />
                            <div style="display:none;margin-top:4px">
                                <span style="color:var(--iq-secondary-dark)"> <?php echo esc_html__("Invalid file, Please select file size upto ","kc-lang") .esc_html($max > 0 ? $max /( 1024*1024) : 0).esc_html__('MB','kc-lang') ?></span>
                            </div>
                        <?php
                    }
                    if($customField['type'] === 'multiselect' && !empty($customField['options'])){
                        ?>
                        <select class="appointment_widget_multiselect iq-kivicare-form-control 
                        <?php echo esc_html(in_array($customField['isRequired'], [1, '1']) ? ' kivicare-required' : ''); ?>"  multiple
                            id="<?php echo esc_html($customField['name'] . '_' . $customField['id']); ?>"
                             placeholder="<?php echo esc_html($customField['placeholder']); ?>"
                             name="custom_field_<?php echo esc_html($customField['id']); ?>" >
                            <?php 
                    ...
                    ?>
                        <select id="<?php echo esc_html($customField['name'] . '_' . $customField['id']); ?>" class="iq-kivicare-form-control text-capitalize <?php echo esc_html(in_array($customField['isRequired'], [1, '1']) ? 'kivicare-required' : ''); ?>" name="custom_field_<?php echo esc_html($customField['id']); ?>">
                        ...
                        <div class="d-flex flex-wrap">
                            ...
                            ?>
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input type="radio" id="<?php echo esc_html($customField['name'] . '_' . $key); ?>" name="custom_field_<?php echo esc_html($customField['id']); ?>" value="<?php echo esc_html($option['text']); ?>" class="custom-control-input <?php echo esc_html(in_array($customField['isRequired'], [1, '1']) ? 'kivicare-required' : ''); ?>">
                    ...
                    ?>
                        <div class="d-flex flex-wrap justify-content-space " style="gap:5px;">
                            ...
                            foreach ($customField['options'] as $key => $option) {
                            ?>
                                <div class="custom-control custom-checkbox custom-control-inline d-flex align-items-center" style="gap:5px;">
                                    <input type="checkbox" id="<?php echo esc_html($customField['name'] . '_' . $key); ?>" name="custom_field_<?php echo esc_html($customField['id']); ?>" value="<?php echo esc_html($option['id']); ?>" class="custom-control-input <?php echo esc_html(in_array($customField['isRequired'], [1, '1']) ? 'kivicare-required' : ''); ?>">
                                    <label class="custom-control-label" for="<?php echo esc_html($customField['name'] . '_' . $key); ?>">
                                        <?php echo esc_html($option['text']); ?>
                                    </label>
                                </div>
                    ...
                    ?>
                        <textarea id="<?php echo esc_html($customField['name'] . '_' . $customField['id']); ?>" name="custom_field_<?php echo esc_html($customField['id']); ?>" placeholder="<?php echo esc_html($customField['placeholder']); ?>" class="iq-kivicare-form-control <?php echo esc_html(in_array($customField['isRequired'], [1, '1']) ? 'kivicare-required' : ''); ?>"></textarea>
                    <?php
                    }
...
function kcAllowedHtml() {
    ...
    function kcEmailContentKeyReplace($content, $data)
    {
        ...
        $keys = [
            ...
            '{{appointment_date}}' => isset($data['appointment_date']) ? esc_html(date_i18n(get_option('date_format'), strtotime($data['appointment_date']))) : '',
        ...
        if (count($clinic_session)) {
            ...
            foreach ($clinic_session as $key => $session) {
                    ...
                    if (
                        ...
                        if ($current_time_slot_start == $session_start_time) {
                            ...
                            $chuck_session[$last_element] = [
                                'start' => $chuck_session[$last_element]['start'], 'end' => $time->appointment_start_time
                            ];
                ...
                foreach ($chuck_session as $chunk) {
                    ...
                    if (!($start_time_formated >= $session_start_time && $start_time_formated <= $session_end_time
                ...
                if (!empty($appointment_data) && is_array($appointment_data) && count($appointment_data) > 0) {
                    ...
                    foreach ($appointment_data as $appoint) {
                        ...
                        if (
                            ...
                            $temp['email_status'] = kcSendEmail($data) ? 1 : 0;
                        }
                        if (
    ...
    function kivicareWoocommerceOrderDataAfterOrderDetails($order)
    {
        ...
        <p class="form-field form-field-wide wc-order-status kivicare-orderinfo">
            ...
            <label for="kivicare_appointment_print">
                ...
                <button class="button button-primary kivicare-order-print-appointment-disabled" style="display:none; cursor: not-allowed;pointer-events: none;">
                    <strong>
                        <?php
                        /* translators: Appointment*/
...
?>
    <div>
        <link href="<?php echo esc_js(KIVI_CARE_DIR_URI . 'assets/css/bootstrap.min.css' . '?v=' . KIVI_CARE_VERSION); ?>" rel="stylesheet" type="text/css" media="print" />
        <style>
            @media print {
                .container-kivicare {
                    font-size: 1rem;
                ...
                .table-bordered th,
                .table-bordered td {
                    border-color: <?php echo esc_attr($themeColor); ?>;
                }
                ...
                .border-right-5 {
                    border-right: 5px solid <?php echo esc_attr($themeColor); ?>;
                }

                .border-top-5 {
                    border-top: 5px solid <?php echo esc_attr($themeColor); ?>;
                }
                ...
                table .border-primary {
                    border-color: <?php echo esc_attr($themeColor); ?> !important;
                }
            ...
            <header>
                <div class="header-top">
                    <div class="container-fluid">
                        <?php do_action('kivicare_print_before_clinic_data', $data, $id) ?>
                        <div class="row m-2 mt-4">
                            <div class="col-3">
                                <img src="<?php echo esc_url($data->clinic_logo); ?>" width="100px" height="100px" class="rounded">
                            </div>
                            ...
                            <div class="col-3 d-flex justify-content-end align-items-center">
                                <p class="mb-0">
                                    <span class="text-dark">
                                        <?php echo esc_html__('Date:', 'kc-lang') . esc_html($data->date); ?>
                                    </span>
                                </p>
                            </div>
                        </div>
                            ...
                            <div class="col-8">
                                <div class="row mt-2">
                                    <div class="col-12">
                                        <p class="mb-0">
                                            <span class="text-dark">
                                                <?php echo esc_html__('Dr.', 'kc-lang') . esc_html($data->doctor_name); ?>
                                            </span>
                                            <?php echo !empty($data->qualifications) ? esc_html__('Qualifications:', 'kc-lang') . esc_html($data->qualifications) : ''; ?>
                                        </p>
                                    </div>
                                </div>
                                <div class="row mt-2">
                                    <div class="col-12">
                                        <p class="mb-0">
                                            <span class="text-dark">
                                                <?php echo esc_html__('Address: ', 'kc-lang'); ?>
                                            </span>
                                            <?php echo esc_html($data->clinic_address); ?>
                                        </p>
                                        <p class="mb-0" style="display: <?php echo  esc_html($show_invoice_id); ?>">
                                            <span class="text-dark">
                                                <?php echo esc_html__('Invoice Id: ', 'kc-lang'); ?>
                                            </span>
                            ...
                            <div class="col-4">
                                <div class="row mt-2">
                                    <div class="col-12 d-flex justify-content-end">
                                        <p class="mb-0">
                                            <span class="text-dark">
                                                <?php echo esc_html__('Contact No: ', 'kc-lang'); ?>
                                            </span>
                                ...
                                <div class="row mt-2">
                                    <div class="col-12 d-flex justify-content-end">
                                        <p class="mb-0">
                                            <span class="text-dark">
                                                <?php echo esc_html__('Email: ', 'kc-lang'); ?>
                                            </span>
                        ...
                        <div class="row m-2">
                            <div class="col-6">
                                <p class="mb-0">
                                    <span class="text-dark">
                                        <?php echo esc_html__('Patient Name:', 'kc-lang'); ?>
                                    </span>
                            ...
                            <div class="col-3">
                                ...
                                ?>
                                    <p class="mb-0">
                                        <span class="text-dark">
                                            <?php echo esc_html__('Age:', 'kc-lang'); ?>
                                        </span>
                            ...
                            <div class="col-3 d-flex justify-content-end">
                                ...
                                ?>
                                    <p class="mb-0">
                                        <span class="text-dark">
                                            <?php echo esc_html__('Gender:', 'kc-lang'); ?>
                                        </span>
                        ...
                        <div class="row m-2">
                            ...
                            <div class="col-6 d-flex justify-content-end">
                                <?php if (!empty($data->contact_no)) {
                                ?>
                                    <p class="mb-0">
                                        <span class="text-dark">
                                            <?php echo esc_html__('Contact No:', 'kc-lang'); ?>
                                        </span>
                                        <?php echo esc_html($data->contact_no); ?>
                                    </p>
                                <?php
                                } ?>
                            </div>
                        </div>
                        <?php do_action('kivicare_print_after_patient_data', $data, $id, $current_user_role); ?>
                    </div>
                </div>
            </header>
    ...
    <?php
        ...
        {
            $tax_details = apply_filters('kivicare_calculate_tax',['status' => false,
                'message' => '',
                'data' => []
                ],[
                    "id" => $id,
                ...
                <table class="table table-bordered border-primary">
                    ...
                    <tbody>
                        ...
                        <?php if(!empty($tax_details['tax_total'])){
                            ?>
                            <tr>
                                <th colspan="4" class="border-primary text-right text-dark">
                                    <?php echo esc_html__('Total', 'kc-lang'); ?>
                                </th>
                                <td class="border-primary">
                            ...
                            <tr>
                                <th colspan="4" class="border-primary text-right text-dark">
                                    <?php echo esc_html__('Tax', 'kc-lang'); ?>
                                </th>
                                <td class="border-primary">
                        ...
                        <tr>
                            <th colspan="4" class="border-primary text-right text-dark">
                                <?php echo esc_html__('Discount', 'kc-lang'); ?>
                            </th>
                            <td class="border-primary">
                        ...
                        <tr>
                            <th colspan="4" class="border-primary">
                                <div class="d-flex align-items-center justify-content-between">
                                    <span class="text-left text-dark"><?php echo esc_html__('Payment Status: ', 'kc-lang'); ?>
                                        <span class="badge  <?php echo $data->payment_status === 'paid' ?  esc_html('badge-success border-success') :  esc_html('badge-danger border-danger'); ?> " style="font-size:15px;">
                                            <?php echo $data->payment_status == 'paid' ? esc_html__('Paid', 'kc-lang') : esc_html__('Unpaid', 'kc-lang') ?>
                                        </span>
                                    </span>
                                    <span class="text-right text-dark">
                                        <?php echo esc_html__('Amount due', 'kc-lang'); ?>
                                    </span>
                                </div>
                            </th>
                            <td class="border-primary">
                ...
                <div class="row m-2">
                    <div class="col-12">
                        <p class="mb-0" style="word-wrap: break-word">
                            ...
                            <a href="<?php echo esc_url($meetingJoinLink); ?>" target="_blank"><?php echo esc_html($meetingJoinLink); ?></a>
                        </p>
                    </div>
                </div>
            <?php
                ...
                <div class="row m-2">
                    <div class="col-12">
                        <p class="mb-0" style="word-wrap: break-word">
                            ...
                            <a href="<?php echo esc_url($meetingStartLink); ?>" target="_blank"><?php echo esc_html($meetingStartLink); ?></a>
                        </p>
                    </div>
                </div>
            <?php
    ...
    <?php
        ...
        {
    ?>
    <div class="row m-2 text-center mb-4 mt-4">
                ...
                ?>
                    <div class="col-4">
                        <p class="mb-0 d-flex justify-content-center align-items-center">
                            ...
                            ?>
                                <input type="checkbox" class="mr-1" id="<?php echo esc_html($cValue['id']); ?>" <?php echo esc_html(in_array($cValue['text'], $customValue['field_data']) ? 'checked' : ''); ?> readonly style="height: unset;margin-right:unset;accent-color:<?php echo esc_html($themeColor); ?>;vertical-align:unset">
    ...
    <div class="card-widget-footer">
        <span id="<?php echo esc_html($type); ?>" class="alert alert-popup alert-danger alert-left error" style="display:none">&nbsp;</span>
        <div class="d-flex justify-content-end gap-1-5 " style="margin-left: auto;">
            <?php if ($currentActive !== 'active') { ?>
                <button type="button" class="iq-button iq-button-secondary" id="iq-widget-back-button" data-step="prev"><?php echo esc_html__('Back', 'kc-lang'); ?></button>
            <?php } ?>
            <?php if ($type == 'kivicare_error_msg_confirm_next') { ?>
                <button type="button" id="kivicare_confirm_next" class="iq-button iq-button-primary"><?php echo esc_html($buttonText); ?></button>
            <?php } else { ?>
                <button type="submit" name="submit" value="submit" data-step="next" class="iq-button iq-button-primary"><?php echo esc_html($buttonText); ?></button>
            <?php } ?>
        </div>
    </div>
<?php
            ...
            if (count($clinic_sessions)) {
                ...
                foreach ($clinic_sessions as $session) {
                    ...
                    if ($session->parent_id === null || $session->parent_id === "") {
                        ...
                        $morning_session = !empty($session->start_time && $session->end_time) ? kcGetFormatedTime(date('H:i', strtotime($session->start_time))) . ' to ' . kcGetFormatedTime(date('H:i', strtotime($session->end_time))) : "-";
...
/**
 * Delete custom form data associated with a patient encounter when the 'kivicare_encounter_delete' action is triggered.
 *
 * @param int $id The ID of the patient encounter to delete.
 */
add_action('kivicare_encounter_delete', function ($id) {
    // Trigger the 'kivicare_custom_form_data_delete' action for the 'patient_encounter_module' with the encounter's ID.
    do_action('kivicare_custom_form_data_delete', 'patient_encounter_module', $id);
});

/**
 * Delete custom form data associated with an appointment when the 'kc_appointment_delete' action is triggered.
 *
 * @param int $id        The ID of the appointment to delete.
 * @param int $encounter The ID of the associated encounter.
 */
add_action('kc_appointment_delete', function ($id, $encounter) {
    // Trigger the 'kivicare_custom_form_data_delete' action for the 'appointment_module' with the appointment's ID.
    do_action('kivicare_custom_form_data_delete', 'appointment_module', $id);
}, 10, 2);
