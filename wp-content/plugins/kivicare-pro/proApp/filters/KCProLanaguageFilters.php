...
class KCProLanaguageFilters extends KCBase {
    ...
    public function commonSendSms($receiverNo,$body){
        ...
        if (!empty($sid) && !empty($token) && !empty($receiverNo) && !empty($get_sms_config->to_number)) {
            try {
                ...
                $status = $twilio->messages->create($receiverNo, // to
    ...
    public function commonWhatsAppMessageSend($receiverNo, $body) {
        ...
        if (!empty($sid) && !empty($token) && !empty($receiverNo) && !empty($get_whatsapp_config->wa_to_number)) {
            try {
                    $twilio = new Client($sid, $token);
                    $status = $twilio->messages->create("whatsapp:".str_replace(' ', '', $receiverNo), // to
                        array(
    ...
    public function sendCustomNotification($type, $receiverNo, $body, $data)
    {
        ...
        $dynamicKeysData = str_replace(['[dynamic_sms_body]', '[dynamic_receiver_number]'], [$body, $receiverNo], $dynamicKeysData);
        ...
        if ($customNotificationConfig['method'] === 'GET') {
            ...
            if (!empty($customNotificationConfig['body'])) {
                $args['body'] = json_decode($customNotificationConfig['body'],true)??$customNotificationConfig['body'];
            }
            $args = apply_filters('kcpro_custom_notification_api_args',$args);
