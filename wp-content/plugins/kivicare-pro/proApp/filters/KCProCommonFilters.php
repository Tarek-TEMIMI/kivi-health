...
class KCProCommonFilters extends KCBase {
    ...
    public function verifySelectTimeslot($request_data){
        ...
        if(!empty($service_id)){
            ...
            $service_duration = $wpdb->get_var("SELECT SUM(duration) FROM {$wpdb->prefix}kc_service_doctor_mapping WHERE service_id IN ({$service_id}) AND doctor_id={$request_data['doctor_id']['id']} AND clinic_id={$request_data['clinic_id']['id']}");
        }
    ...
    public function update_user_choose_language($default_value,$request) {
        ...
        if($request['choose_language'] !== get_user_locale() ||  $request['choose_language'] !== get_locale()){
